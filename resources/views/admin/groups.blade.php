  ...
  <div class="container mx-auto p-6">
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex gap-3">
        <form method="GET" action="{{ route('admin.groups') }}" class="flex items-center">
        ...
        <button id="open-modal" class="flex items-center gap-2 px-4 py-2 text-white rounded-lg"
          style="background-color:#123456;">Add Group</button>
      </div>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full table-fixed">
          <thead style="background-color:#abcdef;">
            <tr>
              <th class="px-6 py-3 text-left w-1/4">Group Name</th>
              <th class="px-6 py-3 text-left w-1/3">Description</th>
              <th class="px-6 py-3 text-left w-1/6">Links</th>
              <th class="px-6 py-3 text-left w-1/4">Actions</th>
            </tr>
          </thead>
          <tbody>
            @foreach($groups as $group)
              <tr class="{{ $loop->even ? 'bg-gray-50' : 'bg-white' }}">
                <td class="px-6 py-4 truncate" title="{{ $group->name }}">{{ $group->name }}</td>
                <td class="px-6 py-4 truncate" title="{{ $group->description }}">{{ $group->description }}</td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4">
                  <div class="flex items-center gap-2">
                    <button class="edit-btn px-2 py-1 border rounded text-sm whitespace-nowrap" data-id="{{ $group->id }}"
                      data-name="{{ e($group->name) }}" data-description="{{ e($group->description) }}"
