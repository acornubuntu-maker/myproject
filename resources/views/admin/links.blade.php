  ...
  <div class="container mx-auto p-6">
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex gap-3">
        <form method="GET" action="{{ route('admin.links') }}" class="flex items-center">
          <input type="text" name="search" placeholder="Search links..." value="{{ request('search') }}"
        ...
        <button id="open-modal" class="flex items-center gap-2 px-4 py-2 text-white rounded-lg"
          style="background-color:#123456;">Add Link</button>
      </div>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full table-fixed">
          <thead style="background-color:#abcdef;">
            <tr>
              <th class="px-6 py-3 text-left w-1/4">Title</th>
              <th class="px-6 py-3 text-left w-1/3">Description</th>
              <th class="px-6 py-3 text-left w-1/4">URL</th>
          ...
          <tbody>
            @foreach($links as $link)
              <tr class="{{ $loop->even ? 'bg-gray-50' : 'bg-white' }}">
                <td class="px-6 py-4 truncate" title="{{ $link->title }}">{{ $link->title }}</td>
                <td class="px-6 py-4 truncate" title="{{ $link->description }}">{{ $link->description }}</td>
                <td class="px-6 py-4 truncate" title="{{ $link->url }}">
                ...
                <td class="px-6 py-4">
                  <div class="flex items-center gap-2">
                    <button class="edit-btn px-2 py-1 border rounded text-sm whitespace-nowrap" data-id="{{ $link->id }}"
                      data-title="{{ e($link->title) }}" data-description="{{ e($link->description) }}"
                      data-url="{{ e($link->url) }}">Edit</button>
